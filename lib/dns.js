...
DNSParser = function (buffer) {
  ...
  var parseQname = function () {
    ...
    while (true) {
      ...
      if ( (len[0] & 0xC0) != 0) {
        ...
        name.push( nameCache[offset] )
      } else {
        name.push( take(len[0]).toString('ascii') );
      }
    } 
    console.log(opos + " : " + name)
    return name
  }

  /*
   * Apparently, compressed NAMEs can point to (for all intents
   * and purposes) arbitrary positions within the packet.
   * this grabs a NAME from an offset and more or less
   * copy and pastes the parseQname function above w/out
   * handling compression (because hopefully, compression doesn't
   * chain)
   *
   * Fuck this! compression does chain...
   */
  var parseCompressed = function(offset) {
    ...
    while(true) {
      ...
      console.log("len: "+len)
      console.log("blen: "+buffer.length)
      console.log("pos: "+pos)
      console.log("nsf: "+name)
      if (len == 0x00) {
        break;
      }
      console.log(buffer)
